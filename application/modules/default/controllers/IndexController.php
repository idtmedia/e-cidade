...
class Default_IndexController extends Default_Lib_Controller_AbstractController {
  ...
  public function autenticaAction() {
    ...
    $this->view->oPrestador       = $oContribuinte;
    $this->view->sLogoPrestador   = Administrativo_Model_Empresa::getLogoByIm($this->view->oDadosNota->getP_im());
    ...
    if ($oNota->getIdNotaSubstituida()) {
      $this->view->oDadosNotaSubstituida = Contribuinte_Model_Nota::getById($oNota->getIdNotaSubstituida());
    }
    ...
    if (in_array($this->view->oDadosPrefeitura->getModeloImpressaoNfse(), array(2, 3))) {
      ...
      $this->view->qrcode = DBSeller_Helper_QRCode_QRCode::getQrCodeNfse($this->view->serverUrl(),
                                                                         $sCodigoVerificacao,
                                                                         $this->view->oDadosNota->getT_cnpjcpf());
    }

    $this->view->setBasePath(APPLICATION_PATH . '/modules/contribuinte/views/');

    $sHtml = $this->view->render("pdf/nota_modelo_{$this->view->oDadosPrefeitura->getModeloImpressaoNfse()}.phtml");

    echo "<body style='margin:auto'>{$sHtml}</body>";
  }

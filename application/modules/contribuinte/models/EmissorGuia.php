...
class Contribuinte_Model_EmissorGuia {
  ...
  /**
   * Verifica se deve emitir a guia considerando as características do contribuinte
   * 
   * @param stdClass $oParametroStdClass
   *   $oParametroStdClass->inscricao_municipal
   *   $oParametroStdClass->data
   * @throws Exception
   * @return boolean
   * 
   * @tutorial: Abaixo constam as regras para emissão de guia
   * 
   * Regime(4)/Exigibilidade(5) |Exigível(23)|Ñ Incide(24)|Isento(25)|Export.(26)|Imune(27)|Susp.Judic(28)|Susp.Adm(29)
   * ------------------------------------------------------------------------------------------------------------------
   ...
   */
  public static function checarEmissaoGuia($oParametroStdClass = NULL) {
    
    $oValidaData = new Zend_Validate_Date();
    
    if (!is_object($oParametroStdClass)) {
      throw new Exception('O parâmetro informado deve ser um objeto.');
    } else if (!isset($oParametroStdClass->data) || !isset($oParametroStdClass->inscricao_municipal))  {
      throw new Exception('Verifique os parâmetros informados.');
    } else if (!$oValidaData->isValid($oParametroStdClass->data)) {
      throw new Exception('Parâmetro "data" inválido.');
    }
    
    // Busca os dados do contribuinte da sessão
    $oContribuinte = Contribuinte_Model_Contribuinte::getByInscricaoMunicipal($oParametroStdClass->inscricao_municipal);
    ...
    $oDataSimples = new DateTime(DBSeller_Helper_Date_Date::invertDate($oParametroStdClass->data));

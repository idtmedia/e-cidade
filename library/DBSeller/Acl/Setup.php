...
class DBSeller_Acl_Setup {
  ...
  /**
   * Captura a versão do ecidade
   */
  protected function capturaversaoEcidade () {
    
    $oSessao = new Zend_Session_Namespace('sessao');
    
    $aCampos = array('ecidadeVersao');
    $oWebService = E2Tecnologia_Model_WebService::consultar('getVersaoEcidade', array(array(), $aCampos));
    
    $sVersaoEcidade = $oWebService[0]->ecidadeversao;

    $oVersaoNfse = Administrativo_Model_Versao::getByAttribute('ecidade', $sVersaoEcidade);

    $oSessao->versaosistema = 0;

    if (!empty($oVersaoNfse)) {
      $oSessao->versaosistema = $oVersaoNfse->getVersaoEcidadeOnline2();
    }

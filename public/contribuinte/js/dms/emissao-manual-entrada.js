$(function() {
  ...
  $oBtnLancarServico.live('click', function(e) {
    ...
    $.ajax({
      ...
      beforeSend: function() {
        $oThis.val('Aguarde...').attr('disabled', true);
      },
      complete  : function() {
        $oThis.val(txtElm).attr('disabled', false);
      },
      error     : function(xhr) {
        ...
        if (xhr.responseText) {
          ...
          bootbox.alert('Ocorreu um erro.<br>readyState: ' + xhr.readyState + '<br>status: ' + xhr.status);
        }
      },
      success   : function(data) {
